...
{
    public partial class GreatlakesIndustriesAppProductUserControl : UserControl
    {
        string indName = string.Empty;
        public GreatlakesIndustriesAppProduct MyVisualWebpart { get; set; }//get dynamic url value
        ...
        public string GetPDF(object pdfUrl)
        {
            ...
            for (int i = 0; i < pdfvalue.Length; i++)
            {
                ...
                {
                    if (pdfvalue[i].ToString().Contains("Technical Data Sheet - Trutint 50"))
                    {
                        originalValue = pdfvalue[i].ToString().Split(';')[1];
                        return originalValue;
                    }
                    else if (pdfvalue[i].ToString().Contains("Technical Data Sheet"))
                    {
                        originalValue = pdfvalue[i].ToString().Split(';')[1];
                    }
                }
            ...
            try
            {
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        ...
                        string strchildpath = string.Empty;// "http://usthvgdev02:5480/sites/greatlakes/Pages/IndustriesAppProd.aspx";
                        ...
                        string strAInd = Page.Request.QueryString["Ind"];
                        string strAppTitle = "";
                        string strAppDesc = "";
                        string strImgPath = "";
                                ...
                                if (strApp != null)
                                {
                                    foreach (SPListItem oitem in spItemCollection) // Loop over the rows.
                                    {
                                        if (oitem["ID"].ToString() == strApp)
                                        {
                                            ...
                                            leftNav.Append("<li class='T2 parent current'><a href=" + strchildpath + "?app=" + oitem["ID"] + " title='" + oitem["Title"] + "'>" + oitem["Title"] + "</a></li>");
                                            breadcrum.Append("<a title='" + oitem["Title"] + "' href='" + strchildpath + "?app=" + oitem["ID"] + "'>" + oitem["Title"] + "</a> &#47;");
                                            ...
                                            foreach (SPListItem ochitem in oItemCollection)
                                            {
                                                ...
                                                {
                                                    if (Convert.ToString(ochitem["Title"]) == strAInd)
                                                    {
                                                        strAppTitle = ochitem["Title"].ToString();
                                                        breadcrum.Append("<a title='" + strAppTitle + "'href='" + pageUrl + "?Ind=" + ochitem["Title"] + "&app=" + oitem["ID"] + "'>" + ochitem["Title"] + "</a> ");
                                                        ...
                                                        desc.Append("<p>" + childdesc + "</p>");
                                                        leftNav.Append(" <li class='T3 current'><a  href='" + pageUrl + "?Ind=" + ochitem["Title"] + "&app=" + oitem["ID"] + "' title='" + Convert.ToString(ochitem["Title"]) + "'>" + Convert.ToString(ochitem["Title"]) + "</a></li>");
                                                    }
                                                    else
                                                        leftNav.Append(" <li class='T3'><a  href='" + pageUrl + "?Ind=" + ochitem["Title"] + "&app=" + oitem["ID"] + "' title='" + Convert.ToString(ochitem["Title"]) + "'>" + Convert.ToString(ochitem["Title"]) + "</a></li>");
                                                }
                                            }
                                        }
                                        else
                                            leftNav.Append("<li class='T2 parent'><a href=" + strchildpath + "?app=" + oitem["ID"] + " title='" + oitem["Title"] + "'>" + oitem["Title"] + "</a></li>");
                ...
                using (SPSite site = new SPSite(SPContext.Current.Site.Url))
                {
                    using (SPWeb web = site.OpenWeb())
                    {
                        ...
                        if (spList != null)
                        {
                            ...
                            {
                                if (indName == "Electrical Components")
                                {
                                    DataTable dtPrinted = spItemCollection.GetDataTable();
                                    DataView dvPrinted = dtPrinted.DefaultView;
                                    dvPrinted.RowFilter = String.Format("Associated_Application LIKE '{0}'", "%Printed Wiring Board%");

                                    if (dvPrinted.Count > 0)
                                    {
                                        region6.Attributes.Add("style", "display:block");
                                        rptPrinted.DataSource = dvPrinted;
                                        rptPrinted.DataBind();
                                    }
                                    DataTable dtConnectors = spItemCollection.GetDataTable();
                                    dtConnectors.DefaultView.RowFilter = String.Format("Associated_Application LIKE '{0}'", "%Connectors%");
                                    if (dtConnectors.DefaultView.Count > 0)
                                    {
                                        region7.Attributes.Add("style", "display:block");
                                        rptConnectors.DataSource = dtConnectors;
                                        rptConnectors.DataBind();
                                    }
                                    region5.Attributes.Add("style", "display:none");
                                }
                                else
                                { 
